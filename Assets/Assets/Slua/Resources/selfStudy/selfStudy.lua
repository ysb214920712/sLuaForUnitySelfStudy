...
function class:init(count)
    ...
    self.colors={Color.red,Color.blue,Color.green,Color.cyan,Color.grey,
                Color.white,Color.yellow,Color.magenta,Color.black}

    for i=0,self.max do
        local cube = GameObject.CreatePrimitive(PrimitiveType.Cube)
        cube.transform.position = Vector3(math.cos(i/self.max*pi*2)*self.r,
                                                    math.sin(i/self.max*pi*2)*self.r,0)
...
function class:update()
    ...
    if not self.fogStart or self.t>1 then
        ...
        self.bgColor = self.colors[maht.random(#self.colors)]
    end
